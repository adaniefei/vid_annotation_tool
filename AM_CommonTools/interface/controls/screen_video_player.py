...
class ScreenVideoPlayer(ScreenElement):
    ...
    def __init__(self, name, width, height):
        ...
        self.click_with_pos_callback = None
    ...
    def on_mouse_button_click(self, pos, button):
        ...
        if button == 1 and self.click_callback is not None:
            self.click_callback(self)
        if self.click_with_pos_callback is not None:
            adjusted_pos = (pos[0] - self.position[0], pos[1] - self.position[1])
            self.click_with_pos_callback(self, button, adjusted_pos)
